...
[data-testid="stChatInput"] textarea { color: #f3e8ff !important; }
...
.astra-welcome p { color: #a78bfa; font-size: 1.1rem; max-width: 500px; }
...
.stTabs [data-baseweb="tab"] { color: #a78bfa !important; border-radius: 8px !important; }
...
.stCodeBlock { border: 1px solid rgba(139, 92, 246, 0.25) !important; border-radius: 10px !important; }
...
def get_connection():
    ...
    if not db_path.exists():
        ...
        with st.spinner("Gerando banco de dados... (so na primeira vez)"):
...
def render_response(data):
    if data.get("type") == "chat":
        ...
        with tab1:
            st.markdown(
                ...
                '<div class="astra-card-header">Astra</div>'
                '<div class="astra-answer">' + data["explanation"] + '</div>'
                '</div>',
                unsafe_allow_html=True,
            )
        return

    tab1, tab2, tab3, tab4 = st.tabs(["Resposta", "Grafico", "SQL", "Dados"])

    with tab1:
        st.markdown(
            '<div class="astra-card">'
            '<div class="astra-card-header">Analise</div>'
            '<div class="astra-answer">' + data["explanation"] + '</div>'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    with tab3:
        formatted_sql = sqlparse.format(
            data["sql"], reindent=True, keyword_case="upper", indent_width=4, wrap_after=80,
        )
...
def process_question(question, conn, agent):
    ...
    with st.chat_message("assistant", avatar="🚀"):
        with st.spinner("Analisando..."):
            try:
                ...
                fallback = {"type": "chat", "explanation": "Erro ao processar. Tente perguntar sobre vendas, produtos, faturamento ou clientes."}
...
SUGGESTIONS = [
    "Quais sao os 10 produtos mais vendidos em 2024?",
    "Qual o faturamento mensal de 2025?",
    "Qual categoria gerou mais receita?",
    "Como as vendas se distribuem por estado?",
    "Qual canal de venda tem maior ticket medio?",
    "Quantos pedidos foram cancelados por mes?",
...
with st.sidebar:
    ...
    st.markdown(
        ...
        'background-clip:text;line-height:1.1;">Astra</div>'
        '<div style="color:#7c3aed;font-size:0.72rem;font-weight:500;'
        'letter-spacing:0.05em;margin-top:0.2rem;">Your AI Data Analyst</div>'
        '</div></div>',
        unsafe_allow_html=True,
    )
    st.divider()
    st.markdown(
        ...
        'letter-spacing:0.07em;text-transform:uppercase;margin-bottom:0.6rem;">Sugestoes</p>',
        unsafe_allow_html=True,
    )
    for suggestion in SUGGESTIONS:
        if st.button(suggestion, key="sug_" + suggestion[:20], use_container_width=True):
...
if not st.session_state.messages:
    st.markdown(
        '<div class="astra-welcome"><h1>Astra</h1>'
        '<p>O que deseja saber sobre seus dados hoje?</p></div>',
        unsafe_allow_html=True,
    )
